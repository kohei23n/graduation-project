...
# データの読み込みと準備
match_data_df = "./csv/match_data.csv"
...
def calc_xg_stats(team, date, df, k=None):
    ...
    for _, match in past_matches.iterrows():
        if match["HomeTeam"] == team:
            ...
            xg_conceded.append(match["AwayXG"], "away")
        else:
            xg.append(match["AwayXG"], "away")
            xg_conceded.append(match["HomeXG"], "home")
...
def add_recent_xg_stats(df, k):
    df["HT_RecentXG"] = 0.0
    df["AT_RecentXG"] = 0.0
    df["HT_RecentXGConceded"] = 0.0
    df["AT_RecentXGConceded"] = 0.0
    ...
    for _, row in df.iterrows():
        ...
        df.at[_, "HT_RecentXG"] = ht_xg
        df.at[_, "HT_RecentXGConceded"] = ht_xg_conceded

        # アウェイチームの直近 k 試合の平均ポイント
        at_xg, at_xg_conceded = calc_xg_stats(away_team, date, df, k)
        df.at[_, "AT_RecentXG"] = at_xg
        df.at[_, "AT_RecentXGConceded"] = at_xg_conceded
...
def add_avg_xg_stats(df):
    df["HT_AvgXG"] = 0.0
    df["AT_AvgXG"] = 0.0
    df["HT_AvgXGConceded"] = 0.0
    df["AT_AvgXGConceded"] = 0.0

    for _, row in df.iterrows():
        home_team, away_team, date = row["HomeTeam"], row["AwayTeam"], row["Date"]

        # ホームチームの累積平均ポイント
        ht_avg_xg, ht_avg_xg_conceded = calc_xg_stats(home_team, date, df, k=None)
        df.at[_, "HT_RecentXG"] = ht_avg_xg
        df.at[_, "HT_RecentXGConceded"] = ht_avg_xg_conceded

        # アウェイチームの累積平均ポイント
        at_avg_xg, at_avg_xg_conceded = calc_xg_stats(home_team, date, df, k=None)
        df.at[_, "AT_RecentXG"] = at_avg_xg
        df.at[_, "AT_RecentXGConceded"] = at_avg_xg_conceded
