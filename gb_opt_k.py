...
# k の最適化の範囲
k_values = range(3, 8)
best_k, best_log_loss, best_rps, best_accuracy = None, float("inf"), float("inf"), 0.0
...
for k in k_values:
    ...
    rps_scores, accuracy_scores, log_losses = [], [], []
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(kf.split(temp_data)):
        ...
        # 評価
        # モデルの予測確率を取得
        y_probs = gb_model.predict_proba(X_val)
        rps_score = evaluate_rps(y_val_encoded, y_probs)

        # Accuracyも計算
        y_pred = gb_model.predict(X_val)
        acc_score = accuracy_score(y_val_encoded, y_pred)

        # Log Lossを計算
        current_log_loss = log_loss(y_val_encoded, y_probs)
        ...
        logging.info(
            f"Fold {fold_idx + 1} - RPS: {rps_score:.4f}, Accuracy: {acc_score:.4f}, Log Loss: {current_log_loss:.4f}"
        )
    ...
    logging.info(
        f"Average RPS: {avg_rps:.4f}, Average Accuracy: {avg_accuracy:.4f}, Average Log Loss: {avg_log_loss:.4f} for k={k}"
    )
...
# 最終データを保存
train_output_path = "./csv/gb_train_data.csv"
test_output_path = "./csv/gb_test_data.csv"
